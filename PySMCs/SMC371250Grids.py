...
##  Modified for SMC251040 grid plot.     JGLi11Nov2022
##  Modified for SMC371250 grid plot.     JGLi16Mar2023
##
"""
...
##  Read global and Arctic part cells. 
    Wrkdir='../tmpfls/'
    DatGMC='../DatGMC/'
    Cel_file = DatGMC+ModelName+'Cels.dat'
    Arc_file = DatGMC+ModelName+'BArc.dat'
...
##  Set plot size and limits and message out anchor point.
    ...
    pzfile=DatGMC+'S3750Vrts'+pltype[0:4]+'.npz'
...
##  Draw your selected grid plot.
    psfile=Wrkdir+ModelName+'grd'+pltype[0:4]+'.ps' 
    bufile='../Bathys/ECBuoys.dat'
    buspct='../Bathys/SPBuoys.dat'

    if( pltype == 'Global'):
        from smcglobl import smcglobl
        smcglobl( cel, nvrts,ncels,svrts,scels,colrs, config,
             mdlname=ModelName, buoys=buspct, psfile=psfile)

    else:
        from smclocal import smclocal
        smclocal( cel, nvrts,ncels,colrs,config, Arctic=Arctic, 
              mdlname=ModelName, psfile=psfile, paprorn=papror)
#             mdlname=ModelName, buoys=bufile, psfile=psfile, paprorn=papror)

    print( " Program finished at %s " % datetime.now().strftime('%F %H:%M:%S') )

## End of SMC371250Grids main program ##
